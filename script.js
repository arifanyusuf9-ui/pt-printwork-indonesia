...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (typeof Scrollytelling !== 'undefined') {
    new Scrollytelling({
      ...
      framesDir: 'animation-compressed',
      frameCount: 150
    });
  }
  ...
  if (hzSticky && hzTrack) {
    let ticking = false;
    let hzStickyHeight = 0;
    let hzTrackScrollWidth = 0;
    let maxTranslateX = 0;
    ...
    function recalculateValues() {
      ...
      hzStickyHeight = hzSticky.offsetHeight || (windowHeight * 5);
      hzTrackScrollWidth = hzTrack.scrollWidth;

      maxTranslateX = hzTrackScrollWidth - windowWidth;

      // Fallback if scrollWidth is not ready or too small
      if (maxTranslateX <= 0 && cards.length > 0) {
        const gap = 40;
        const pad = windowWidth * 0.2;
        const totalCardsWidth = Array.from(cards).reduce((a, b) => a + b.offsetWidth, 0);
        maxTranslateX = (totalCardsWidth + (cards.length - 1) * gap + pad) - windowWidth;
      }
    ...
    function updateHorizontalScroll() {
      // Periodic recalculation fallback if dimensions seem missing
      if (hzStickyHeight === 0 || maxTranslateX <= 0) {
        recalculateValues();
      }
      ...
      const maxScrollY = Math.max(1, hzStickyHeight - windowHeight);
      ...
      if (scrollDistance <= 0) {
        ...
        const translateX = maxTranslateX * scrollPercent;
        hzTrack.style.transform = `translate3d(-${translateX}px, 0, 0)`;
      }
      ticking = false;
    }

    // Initial calculations with delays for stability
    recalculateValues();
    updateHorizontalScroll();
    setTimeout(recalculateValues, 500);
    setTimeout(recalculateValues, 2000);


    // Recalculate on window load and resize to ensure everything is measured correctly
    window.addEventListener('load', () => {
      recalculateValues();
      updateHorizontalScroll();
    });
